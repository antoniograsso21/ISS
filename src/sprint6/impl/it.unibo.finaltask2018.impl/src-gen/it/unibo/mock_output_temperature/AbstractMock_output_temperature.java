...
public abstract class AbstractMock_output_temperature extends QActor implements IActivity{ 
	...
	protected IActorAction action;
		...
		protected static IOutputEnvView setTheEnv(IOutputEnvView outEnvView ){
			EnvFrame env = new EnvFrame( "Env_mock_output_temperature", java.awt.Color.green  , java.awt.Color.black );
			env.init();
			env.setSize(800,430); 
			IOutputEnvView newOutEnvView = ((EnvFrame) env).getOutputEnvView();
			return newOutEnvView;
		}
		public AbstractMock_output_temperature(String actorId, QActorContext myCtx, IOutputEnvView outEnvView )  throws Exception{
			super(actorId, myCtx,  
			"./srcMore/it/unibo/mock_output_temperature/WorldTheory.pl",
			setTheEnv( outEnvView )  , "init");
			addInputPanel(80);
			addCmdPanels();
			this.planFilePath = "./srcMore/it/unibo/mock_output_temperature/plans.txt";
	  	}
		...
		*/    
	    ...
	    StateFun handleToutBuiltIn = () -> {	
	    	try{	
	    		...
	    		println( "mock_output_temperature tout : stops");  
	    ...
	    try{	
	     PlanRepeat pr = PlanRepeat.setUp("init",-1);
	    	...
	    	temporaryStr = "\"Temperature Observer STARTED\"";
	    	println( temporaryStr );  
	    	//switchTo waitForEvents
	        switchToPlanAsNextState(pr, myselfName, "mock_output_temperature_"+myselfName, 
	    ...
	    try{	
	     ...
	     pr.incNumIter(); 	
	    	...
	    	//bbb
	     msgTransition( pr,myselfName,"mock_output_temperature_"+myselfName,false,
	    ...
	    try{	
	     PlanRepeat pr = PlanRepeat.setUp("handleModelChanged",-1);
	    	...
	    	curT = Term.createTerm("modelChanged(resource(name(temp),state(temperature(VALUE))))");
	    	if( currentEvent != null && currentEvent.getEventId().equals("modelChanged") && 
	    		...
	    		pengine.unify(curT, Term.createTerm( currentEvent.getMsg() ) )){ 
	    			String parg = "temp(VALUE)";
	    			/* Print */
	    			parg =  updateVars( Term.createTerm("modelChanged(resource(NAME,STATE))"), 
	    			                    Term.createTerm("modelChanged(resource(name(temp),state(temperature(VALUE))))"), 
	    				    		  	Term.createTerm(currentEvent.getMsg()), parg);
	    			if( parg != null ) println( parg );
	    	}
	    	repeatPlanNoTransition(pr,myselfName,"mock_output_temperature_"+myselfName,false,true);
